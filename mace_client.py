#!/usr/bin/env python3
"""
MACE Client - Helper module for calling MACE MCP server from Backplane.

Provides synchronous wrappers around async MCP calls for easy integration
with the agent workflow.

Note: MCP imports are lazy-loaded to avoid requiring Python 3.10+ in calling code.
"""
...
class MACEClient:
    """Client for calling MACE MCP server with caching and error handling."""
    ...
    async def _call_mace_tool(
        self,
        tool_name: str,
        arguments: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """
        Call MACE MCP tool asynchronously.
        ...
        """
        try:
            # Lazy import MCP to avoid requiring Python 3.10+ at module load time
            ...
            print(f"Error: MCP SDK not available (requires Python 3.10+): {e}")
            print(f"Note: MACE client needs to run MCP server via subprocess")
            return None
    ...
    def predict_energy(
        ...
        return asyncio.run(self._call_mace_tool("calculate_energy", arguments))
    ...
    def predict_forces(
        ...
        return asyncio.run(self._call_mace_tool("calculate_forces", arguments))
    ...
    def predict_properties(
        ...
        return asyncio.run(self._call_mace_tool("predict_properties", arguments))
    ...
    def batch_predict_energies(
        ...
        return asyncio.run(self._call_mace_tool("batch_calculate_energies", arguments))
    ...
    def optimize_geometry(
        ...
        return asyncio.run(self._call_mace_tool("optimize_geometry", arguments))
